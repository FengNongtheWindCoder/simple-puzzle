...
public class PuzzleImgProcessMenu   {
    ...
    static void sliceImgByRowColumn(Texture2D image, int row, int column)
    {
        ...
        int generateUnit = 4;//假设生成后的大小是4unit
        //获取导入的图片，及importer
        ...
        teximporter.spritePixelsPerUnit = image.width / generateUnit;
    ...
    static void generateImgAllPiecesPrefab(Texture2D image)
    {
        ...
        int generateUnit = 4;//设置生成后的大小unit，假设生成后是4*4单元大小
        //将所有sprite添加到父节点，并设置其坐标，因为相对父节点坐标，这里的设置会使父节点在中心作为原点。
        foreach (var item in AssetDatabase.LoadAllAssetsAtPath(path))
        {
            ...
            //从原来的左下为原点，转为中心为原点，同时坐标按照像素每单元的比例缩放
            float newPosX = sprite.rect.center.x / teximporter.spritePixelsPerUnit - generateUnit / 2;
            float newPosY = sprite.rect.center.y / teximporter.spritePixelsPerUnit - generateUnit / 2;
            piecesobject.transform.position = new Vector2(newPosX,newPosY);
